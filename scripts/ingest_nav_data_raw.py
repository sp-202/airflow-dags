...
(
    SELECT 
        ile.[Entry No_]                AS entry_no,
        ile.[Item No_]                 AS item_no,
        ile.[Posting Date]             AS posting_date,
        ile.[Entry Type]               AS entry_type,
        ile.[Source No_]               AS source_no,
        ile.[Document No_]             AS document_no,
        ile.[Description]              AS description,
        ile.[Location Code]            AS location_code,
        ile.[Quantity]                 AS quantity,
        ile.[Remaining Quantity]       AS remaining_quantity,
        ile.[Invoiced Quantity]        AS invoiced_quantity,
        ile.[Global Dimension 1 Code]  AS department,
        ile.[Global Dimension 2 Code]  AS process_center_code,
        ile.[Document Date]            AS document_date,
        ile.[External Document No_]    AS external_doc_no,
        ile.[Document Type]            AS document_type,
        ile.[Document Line No_]        AS document_line_no,
        ile.[Dimension Set ID]         AS dimension_set_id,
        ile.[Qty_ per Unit of Measure] AS qty_per_uom,
        ile.[Unit of Measure Code]     AS uom_code,
        ile.[Purpose]                  AS purpose,
        ile.[Supplier Name]            AS supplier_name,
        ile.[Gen Prod_ Posting Group]  AS gen_prod_posting_group,
        ile.[Vehicle No_]              AS vehicle_no,
        ile.[Project Approval Name]    AS project_approval_name,
        ile.[Capital Repairs]          AS is_capital_repair,
        ile.[Exp_ Date]                AS expiration_date,
        ile.[Trip No_]                 AS trip_no,
        ile.[Inventory Posting Group]  AS inventory_posting_group,
        ile.[User ID]                  AS user_id,
        ile.[Item Type]                AS item_type,
    ...
    WHERE ile.[Entry No_] > {last_entry_no}
) t
"""
